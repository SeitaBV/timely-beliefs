...
def test_dst():
    ...
    assert timedelta_for_x_days_ago_at_y_oclock(
...
def test_dst_bounds():
    ...
    timedelta_bounds = timedelta_for_x_days_ago_at_y_oclock(
    ...
    timedelta_bounds = timedelta_for_x_days_ago_at_y_oclock(None, x=210, y=0, z=tz_str)
    event_start = datetime(1947, 11, 10, 23, 59, 59, tzinfo=utc)
    assert (
        timedelta_bounds[0]
        < timedelta_for_x_days_ago_at_y_oclock(event_start, x=210, y=0, z=tz_str)
        < timedelta_bounds[1]
    )
