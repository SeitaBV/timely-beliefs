...
def test_dst():
    ...
    assert timedelta_x_days_ago_at_y_oclock(event_start, x=1, y=12, z=tz_str) == timedelta(
        hours=13
...
def test_dst_bounds():
    ...
    timedelta_bounds = timedelta_x_days_ago_at_y_oclock(None, x=180, y=23.9999999999, z=tz_str)
    event_start = datetime(1947, 5, 12, 22, 0, 0, tzinfo=utc)  # Date with double daylight saving
    assert timedelta_bounds[0] < timedelta_x_days_ago_at_y_oclock(event_start, x=180, y=23.9999999999, z=tz_str) < timedelta_bounds[1]
    ...
    assert timedelta_bounds[0] < timedelta_x_days_ago_at_y_oclock(event_start, x=210, y=0, z=tz_str) < timedelta_bounds[1]
