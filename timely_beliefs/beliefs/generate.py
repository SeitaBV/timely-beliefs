...
def read_beliefs_from_csv(
    ...
    )  # load the observations (keep cp=0.5)
    return blfs
...
def csv_as_belief(csv_in, tz_hour_difference, n_events=None):
    ...
    sensors = (
        tb.Sensor(name=descr[0], unit=descr[1], event_resolution=timedelta(minutes=15))
        for descr in sensor_descriptions
    )
    # Create BeliefsDataFrame
    for sensor in sensors:
        blfs = read_beliefs_from_csv(
            sensor,
            csv_in,
            source=source_a,
...
def generator(df, beliefSeries, model):
    ...
    raise NotImplementedError(
        "This model functionality has not yet been implemented/ Did you pass the correct model?"
    )
...
def main(
    ...
    if current_time.minute % (resolution_minutes) != 0:
        raise ValueError(
            "Your current_time is not compatible with the event resolution of {} minutes".format(
                resolution_minutes
            )
        )
    # get beliefseries from all the times
    current = get_beliefsSeries_from_event_start(
        df, current_time, current_time, "event_value"
    )
    start = get_beliefsSeries_from_event_start(
        df, start_time, current_time, "event_value"
    )
    last_start = get_beliefsSeries_from_event_start(
        df, last_start_time, current_time, "event_value"
    )
        ...
        if temp_time > last_date or temp_time < first_date:
            ...
            beliefSeries_list += [
                get_beliefsSeries_from_event_start(
                    df, temp_time, current_time, "event_value"
                ).copy()
            ]
...
def mean_std_generator(dfnew, df, start, last_start, current_time):
    ...
    if start < first_date or last_start > last_date:
        raise ValueError(
            "Current implementation cannot operate with dates outside of data"
        )
    diff_list = []
    mean_list = []
        ...
        if temp_time < last_date:
            diff_list += [
                abs(
                    ...
                    - get_beliefsSeries_from_event_start(
                        dfnew, temp_time, current_time, "event_value"
                    ).copy()[0]
                )
            ]
            mean_list += [
...
def show_plot(mean, std, start, end, fixedviewpoint=False):
    ...
    for i, cat in enumerate(reversed(cats)):
        ...
        source.add(y, cat)
        p.patch(
            "x", cat, alpha=0.6, color=pallete[i], line_color="black", source=source
        )
    if fixedviewpoint:
...
print(series)
diff_list, mean_list = mean_std_generator(
    series, df, start_time, last_start_time, current_time
)
