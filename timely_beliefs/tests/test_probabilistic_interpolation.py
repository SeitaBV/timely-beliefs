...
@pytest.mark.parametrize(
    "cp, v",
    [
        pytest.param([0.05, 0.5, 0.95], [0, 5, 20]),  # cdf with three points, range(x) encompasses range(cdf)
        pytest.param([0, 0.5], [1, 2]),  # cdf with only two points
        pytest.param([1], [1]),  # cdf with only one point
        pytest.param([0.2, 0.4], [30, 100]),  # range(cdf) completely on the right of range(x)
        pytest.param([0.2, 0.4], [-30, 100]),  # range(cdf) encompasses range(x)
        pytest.param([0.2, 0.8], [-30, -8]),  # range(cdf) completely on the left of range(x)
        pytest.param([0.05, 0.5, 0.6, 0.95], [0, 5, 5, 20]),  # cdf with duplicate values v
    ]
)
