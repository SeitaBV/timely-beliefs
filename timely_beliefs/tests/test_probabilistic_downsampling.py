...
def multivariate_test_cdfs() -> Tuple[np.ndarray, np.ndarray]:
    ...
    marginal_pdf_p = marginal_pdf_p / marginal_pdf_p.sum(axis=1, keepdims=True) - p_tail/n_outcomes  # Normalize
...
def test_bivariate_aggregation():
    ...
    assert equal_lists(cdf_v, [2])  # Check aggregated outcomes (no chance of sum = 0 or 1)
    assert equal_lists(cdf_p, [1])  # Check cumulative probabilities
    # And for possible outcomes P(v=0)=0.5 and P(v=1)=0.5
    cdf_p, cdf_v = multivariate_marginal_to_univariate_joint_cdf([[.5, 1], [.5, 1]])
    assert equal_lists(cdf_v, [0, 1, 2])  # Check aggregated outcomes (25% chance of sum = 0 or 2, 50% of sum = 1)
...
def test_multivariate_aggregation():
    ...
    # For sum
    marginal_pdfs = [[1 / 3, 1 / 3, 1 / 3], [1 / 4, 1 / 4, 2 / 4], [1 / 6, 1 / 6, 2 / 3]]
    marginal_cdfs = np.cumsum(marginal_pdfs, axis=1)
    cdf_p, cdf_v = multivariate_marginal_to_univariate_joint_cdf(marginal_cdfs, a=10, b=100)
    assert all(np.diff(cdf_v) >= 0) and all(np.diff(cdf_p) >= 0)  # Check for non-decreasing cdf
    assert cdf_v[0] == 30 and cdf_v[-1] == 300  # Check range of aggregated outcomes
    assert cdf_p[0] == 1 / 3 * 1 / 4 * 1 / 6 and cdf_p[-1] == 1  # Check range of cumulative probabilities

    # For mean
    cdf_p, cdf_v = multivariate_marginal_to_univariate_joint_cdf(marginal_cdfs, agg_function=np.mean, a=10, b=100)
    assert all(np.diff(cdf_v) >= 0) and all(np.diff(cdf_p) >= 0)  # Check for non-decreasing cdf
    ...
    R[0, 1] = 0.25
    cdf_p, cdf_v = multivariate_marginal_to_univariate_joint_cdf(marginal_cdfs, copula=ot.NormalCopula(R), a=10, b=100)
    assert all(np.diff(cdf_v) >= 0) and all(np.diff(cdf_p) >= 0)  # Check for non-decreasing cdf
    assert cdf_v[0] == 30 and cdf_v[-1] == 300  # Check range of aggregated outcomes
    assert cdf_p[0] > 1 / 3 * 1 / 4 * 1 / 6 and cdf_p[-1] == 1  # Check range of cumulative probabilities
...
def test_bivariate_aggregation_with_unmatched_bins():
    ...
    cdf_p, cdf_v = multivariate_marginal_to_univariate_joint_cdf(marginal_cdfs_p, marginal_cdfs_v=marginal_cdfs_v)
    assert all(np.diff(cdf_v) >= 0) and all(np.diff(cdf_p) >= 0)  # Check for non-decreasing cdf
    assert cdf_v[0] == 5 + 1.2 and cdf_v[-1] == 7 + 3  # Check range of aggregated outcomes
    assert cdf_p[0] == 1 / 3 * 1 / 4 and cdf_p[-1] == 3/4  # Check range of cumulative probabilities

    # Show equalising bins is irrelevant
    marginal_cdfs_v, marginal_cdfs_p = equalize_bins(marginal_cdfs_v, marginal_cdfs_p)
    cdf_p_2, cdf_v_2 = multivariate_marginal_to_univariate_joint_cdf(marginal_cdfs_p, marginal_cdfs_v=marginal_cdfs_v)
...
def test_marginal_distributions_with_residual_probability():
    ...
    marginal_cdfs = [[1 / 3, 2 / 4, 3 / 4], [1 / 4, 4 / 6, 5 / 6], [1 / 6, 5 / 9, 8 / 9]]
    ...
    assert cdf_p[-1] < 1  # Check that the assumed outcome with cp=1 is not actually returned
    a = cdf_p[-1] - cdf_p[-2]  # The probability of the highest outcome for each of the three variables
    ...
    R = ot.CorrelationMatrix(len(marginal_cdfs))
    R[0, 1] = -0.25
    cdf_p, _ = multivariate_marginal_to_univariate_joint_cdf(marginal_cdfs, copula=ot.NormalCopula(R))
    assert all(np.diff(cdf_p) >= 0)  # Check for non-decreasing cdf
    a = cdf_p[-1] - cdf_p[-2]  # The probability of the highest outcome for each of the three variables
    assert a < b  # Check the expected outcome is now lower (if x1 is high, than x2 is less likely to be high)
    ...
    assert all(np.diff(cdf_p) >= 0)  # Check for non-decreasing cdf
    a = cdf_p[-1] - cdf_p[-2]  # The probability of the highest outcome for each of the three variables
    assert a > b  # Check the expected outcome is now higher (if x1 is high, than x2 is likely to be high, too)
...
def test_multivariate_aggregation_with_unmatched_bins(multivariate_test_cdfs):
    ...
    cdf_p, cdf_v = multivariate_marginal_to_univariate_joint_cdf(marginal_cdf_p, marginal_cdfs_v=marginal_cdf_v)
...
def test_multivariate_aggregation_with_unmatched_bins_and_dependence(multivariate_test_cdfs):
    """Check multivariate aggregation where the outcomes of each variable are completely different,
